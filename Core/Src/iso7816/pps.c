...
uint8_t PPS_Negotiate(SmartCard* sc) {
  ...
  HAL_SMARTCARD_Transmit(sc->dev, pps, 4, PPS_TIMEOUT);
  HAL_SMARTCARD_Receive(sc->dev, ppsr, 4, PPS_TIMEOUT);
  ...
  if (sc->atr.default_protocol == SC_T1) {
    ...
    sc->dev->Init.TimeOutEnable = SMARTCARD_TIMEOUT_ENABLE;
    sc->dev->Init.TimeOutValue = 200;
  }
