...
static app_err_t core_lcd_test() {
  ...
  if (ui_prompt("Brightness test", "Screen is at half brightness. Press is OK if brighter than before, cancel otherwise") != CORE_EVT_UI_OK) {
    return ERR_CANCEL;
  }
...
static app_err_t core_camera_test() {
  ...
  if (memcmp(auth.dev_auth_challenge.dev_auth_challenge.value, "0123456789abcdefABCDEF9876543210", 32)) {
    ui_info("The scanned QR code is invalid", 1);
    return ERR_CANCEL;
  }
...
static app_err_t core_card_test() {
  ...
  if (g_core.keycard.sc.state != SC_READY) {
    ui_info("Couldn't start communication with the card", 1);
    return ERR_CANCEL;
  }

  if (keycard_cmd_select(&g_core.keycard, TEST_AID, TEST_AID_LEN) != ERR_OK) {
    ui_info("Couldn't send SELECT APDU", 1);
    return ERR_CANCEL;
  }
...
static app_err_t core_usb_test() {
  ...
  while (!usb_connected()) {
    ui_info("Plug USB and press OK", 1);
  }
  ...
  if ((APDU_CLA(apdu) != 0xe0) || (APDU_INS(apdu) != INS_GET_APP_CONF)) {
    ui_info("Invalid USB command received", 1);
    return ERR_CANCEL;
  }
...
void core_task_entry(void* pvParameters) {
  ...
  ui_prompt("Test OK", "All tests passed, press any key to reboot to application firmware");
